const Upcoming = (() => {
    async function init() {
        const page = document.getElementById('upcoming-page');
        if (!page) return;
    ...
    async function _loadUpcoming(page) {
        ...
        const sorted = [...movies].sort((a, b) => {
            const dA = a.release_date && a.release_date !== 'TBA' ? new Date(a.release_date) : Infinity;
            const dB = b.release_date && b.release_date !== 'TBA' ? new Date(b.release_date) : Infinity;
            return dA - dB;
        });
    ...
    function _createCard(movie, index) {
        ...
        card.addEventListener('click', () => {
            window.location.href = `details.php?id=${movie.id}`;
        });
        return card;
    }
