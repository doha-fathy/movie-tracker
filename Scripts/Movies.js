const Movies = (() => {
    const SECTIONS = [
        { id: 'daily-trend',  title: '🔥 Daily Trending',  fetcher: () => ApiOps.getTrendingDay()    },
        { id: 'weekly-trend', title: '📈 Weekly Trending', fetcher: () => ApiOps.getTrendingWeek()   },
        { id: 'popular',      title: '⭐ Popular',          fetcher: () => ApiOps.getPopularMovies()  },
        { id: 'top-rated',    title: '🏆 Top Rated',       fetcher: () => ApiOps.getTopRatedMovies() },
    ];
    async function init() {
    ...
    function _buildSectionSkeleton(section) {
        ...
        arrowLeft.addEventListener('click',  () => row.scrollBy({ left: -320, behavior: 'smooth' }));
        arrowRight.addEventListener('click', () => row.scrollBy({ left:  320, behavior: 'smooth' }));
    ...
    async function _loadSection(section) {
        ...
        movies.forEach((movie, i) => row.appendChild(_createCard(movie, i)));
    }
    function _createCard(movie, index) {
        ...
        card.addEventListener('click', (e) => {
            ...
            window.location.href = `index.php?page=details&id=${movie.id}`;
        });
    ...
    async function _addToWatchlist(movie, btn) {
        ...
        try {
            const response = await fetch('watchlist_api.php', {
                ...
                body:    JSON.stringify({ action: 'add', movie_id: movie.id })
            });
